...
fn main() {
    ...
    // File sizes
    println!("\n=== File Sizes ===");
    println!("  EPUB input:  {:>8} bytes", std::fs::metadata(epub_path).unwrap().len());
    println!("  AZW3 input:  {:>8} bytes", std::fs::metadata(azw3_path).unwrap().len());
    println!("  AZW3 output: {:>8} bytes", std::fs::metadata("/tmp/profile_out_0.azw3").unwrap().len());
    println!("  EPUB output: {:>8} bytes", std::fs::metadata("/tmp/profile_out_0.epub").unwrap().len());
}
