...
pub static AID_ATTR_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r#"\s+aid\s*=\s*["'][^"']*["']"#).unwrap());

/// Matches data-AmznRemoved attributes
pub static AMZN_REMOVED_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r#"\s+data-AmznRemoved[^=]*\s*=\s*["'][^"']*["']"#).unwrap());

/// Matches data-AmznPageBreak attributes
pub static AMZN_PAGE_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r#"\s+data-AmznPageBreak\s*=\s*["'][^"']*["']"#).unwrap());

/// Matches <img ...> tags for cleanup
pub static IMG_TAG_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r#"<img\s+([^>]*?)(\s*/?>)"#).unwrap());

/// Matches <meta charset="..."> tags
pub static META_CHARSET_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r#"<meta\s*charset\s*=\s*["']([^"']+)["']\s*/?\s*>"#).unwrap());

/// Matches @font-face rules with placeholder URLs
pub static FONTFACE_PLACEHOLDER_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r#"@font-face\s*\{[^}]*url\s*\(\s*X{10,}\s*\)[^}]*\}"#).unwrap());

/// Matches kindle:embed:XXXX references
pub static KINDLE_EMBED_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r#"kindle:embed:([0-9A-V]+)(\?[^"')]*)?["']?"#).unwrap());

/// Matches id="..." attributes in HTML
pub static ID_ATTR_RE: LazyLock<Regex> =
